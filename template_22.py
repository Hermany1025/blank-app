...
if "debater_A" not in st.session_state:
    st.session_state.debater_A = False
if "debater_B" not in st.session_state:
    st.session_state.debater_B = False
if "trait_1" not in st.session_state:
    st.session_state.trait_1 = False
if "trait_2" not in st.session_state:
    st.session_state.trait_2 = False
if "style1" not in st.session_state:
    st.session_state.style1 = False
if "style2" not in st.session_state:
    st.session_state.style2= False
if "topic" not in st.session_state:
    st.session_state.topic = False
if "submit" not in st.session_state:
     st.session_state.submit = False
if "orm" not in st.session_state:
     st.session_state.orm = False
if "net" not in st.session_state:
    st.session_state.net = False
if not st.session_state.orm:
    with st.form ("Form"):
        st.session_state.debater_A = st.text_input("Choose a debater of a character: ", key = "1")
        st.session_state.debater_B = st.text_input("Choose a debater of a character: ", key = "2")
        st.session_state.orm = True
        st.session_state.net = st.form_submit_button("Continue")
if st.session_state.net:
    if (st.session_state.debater_A or st.session_state.debater_B) == "":
        st.error("Missing Info?")
    else:
        with st.form ("form"):
            st.session_state.trait_1 = st.text_input(st.session_state.debater_A + "'s trait", key = "3")
            st.session_state.trait_2 = st.text_input(st.session_state.debater_B + "'s trait", key = "4")
            st.session_state.style1 = st.text_input(st.session_state.debater_A + "'s trait", key = "5")
            st.session_state.style2 = st.text_input(st.session_state.debater_B + "'s trait", key = "6")
            st.session_state.topic = st.text_input("Coose a topic: ", key = "7")
            st.session_state.submit = st.form_submit_button("TIMBUS")
if st.session_state.submit:
    if (st.session_state.debater_A or st.session_state.debater_B or st.session_state.trait_1 or st.session_state.trait_2 or st.session_state.style1 or st.session_state.style2 or st.session_state.topic) == "":
        st.error("Missing Info?")
    else:
        A_prompt = f'''
        You are {st.session_state.debater_A}, a {st.session_state.trait_1} debater.
        You are arguing {st.session_state.debater_B} the topic: {st.session_state.topic}.
        Your debate style is ***{st.session_state.style1}***.
        Follow these rules:
        ...
        - Use ***{st.session_state.style1}*** tone in your responses
        ...
        '''
        B_prompt = f'''
        You are {st.session_state.debater_B}, a {st.session_state.trait_2} debater.
        You are arguing {st.session_state.debater_A} the topic: {st.session_state.topic}.
        Your debate style is ***{st.session_state.style2}***.
        Follow these rules:
        - Present logical arguments with supporting evidence
        - Directly address points made by your opponent
        - Stay on topic and maintain your position
        - Use ***{st.session_state.style2}*** tone in your responses
        - Keep responses between 3-5 sentences
        - Do not concede your position, but acknowledge valid counterpoints
        Each response should include:
        - A main argument or counterargument
        ...
        chat_history_B = [
            ...
            {"role": "user", "content": f"The topic is: {st.session_state.topic}. Your opponent says: {message_A}. Respond with your opening statement."}
        ]
        ...
        st.subheader(f"Opening Statement")
        st.divider()
        ...
        st.write(st.session_state.debater_A)
        ...
        st.write(st.session_state.debater_B)
        ...
        st.divider()
        st.subheader(f"Closing Statement")
        st.divider()
